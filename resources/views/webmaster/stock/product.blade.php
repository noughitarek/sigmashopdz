...
@section('content')
<form action ="{{route('webmaster_stock_store_for_product', $data['product']->id)}}" method="POST"> 
    @csrf
    <div class="row">
        ...
        <div class="col-12 col-lg-12 col-xxl-12 d-flex">
            <div class="card flex-fill">
                ...
                <div class="mb-3">
                        ...
                        <select class="form-select mb-3" name="product" disabled>
                            @foreach($data["products"] as $product)
                            <option value="{{$product->id}}" {{$product->id==$data['product']->id?"selected":""}}>{{$product->name}}</option>
                            @endforeach
                        </select>
                        @error('product')
                        <div class="text-danger">{{ $message }}</div>
